  ...
  const experiences = [
    {
      ...
      icon: "🏢",
      accent: "#0d9488",
      description: "Développement d’une plateforme intelligente de gestion des Stagiaires et du suivi des stages, avec applications web et mobile, backend Spring Boot et module IA d’analyse de CV.",
      tasks: [
    ...
    },
    {
      ...
      icon: "🤖",
      accent: "#0369a1",
      description: "Développement d'un site web de génération automatique de résumés basé sur l'IA avec le modèle NLP T5.",
      tasks: [
    ...
    },
    {
      ...
      icon: "📊",
      accent: "#6d28d9",
      description: "Conception d'un système d'analyse de données académiques pour le suivi des performances des étudiants.",
      tasks: [
  ...
  return (
    <>
      <style>{`
        ...
        .exp-section { padding: 88px 0; background: #f8fafc; font-family: 'Plus Jakarta Sans', sans-serif; }
        .exp-container { max-width: 900px; margin: 0 auto; padding: 0 28px; }
        .section-eyebrow {
          font-size: 11px; font-weight: 700; letter-spacing: 0.18em; text-transform: uppercase;
          color: #0d9488; margin-bottom: 10px; display: flex; align-items: center; gap: 8px;
        }
        ...
        .exp-badge { display: inline-block; padding: 3px 12px; border-radius: 100px; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 8px; }
        .exp-title { font-size: 18px; font-weight: 700; color: #0f172a; margin-bottom: 3px; }
        .exp-company { font-size: 13.5px; font-weight: 600; margin-bottom: 2px; }
        .exp-location { font-size: 12px; color: #94a3b8; margin-bottom: 2px; }
        ...
        .exp-task { display: flex; align-items: flex-start; gap: 8px; padding: 10px 14px; background: #f8fafc; border: 1px solid #f1f5f9; border-radius: 10px; font-size: 13px; color: #374151; line-height: 1.5; }
        .exp-task-icon { font-size: 16px; flex-shrink: 0; }
        .stack-wrap { display: flex; flex-wrap: wrap; gap: 7px; }
        .stack-tag { padding: 4px 12px; border-radius: 100px; font-size: 11.5px; font-weight: 600; }
        @media (max-width: 600px) { .exp-tasks { grid-template-columns: 1fr; } .exp-card { padding: 20px; } }
      `}</style>
            ...
            {experiences.map((exp, i) => (
              <div key={i} className="exp-card" style={{ "--accent": exp.accent }}>
                <div className="exp-top">
                  <div>
                    <span className="exp-badge" style={{ color: exp.badgeColor, background: exp.badgeBg }}>{exp.badge}</span>
                ...
                <div className="stack-wrap">
                  {exp.stack.map((s) => (
                    <span key={s} className="stack-tag" style={{ background: exp.accent + "15", color: exp.accent }}>{s}</span>
                  ))}
                </div>
              </div>
            ))}
